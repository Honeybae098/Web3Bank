...
const AppContent = () => {
  ...
  if (error) {
    return <AppError error={error} onRetry={() => setError(null)} />;
  }
  ...
  const renderPage = () => {
    try {
      ...
      setError(pageError.message || 'Failed to render page');
      return <AppError error={pageError.message} onRetry={() => setError(null)} />;
    }
  };
